...
# cost_fn - h(n) + g(n) value
# cost_gn - g(n) value
class matrixNode:
    
    def __init__(self, level, zeroValIndex, matrix, parentMatrix, currentMatrix, action, changed_tile_val, cost_fn, cost_gn):
        self.level = level
...
class aStarMethodDumpFile():
    ...
    def printSolutionSteps(matrixRootNode):
        ...
        if matrixRootNode == "null":
            return
        aStarMethodDumpFile.printSolutionSteps(matrixRootNode.matrix)  
        if matrixRootNode.action != "Start":
            print(matrixRootNode.action)

    # Main code starts from here!
    def aStar(self, startState, goalState, zeroValIndex,f):

        f.write("Method Selected: a*")
        f.write("\nRunning a*")
        f.write("\n")
        heap = []
        closed_set = []
        # f(n) = h(n) + g(n)
        cost_fn = self.generateCostFunction(startState,goalState) + 0
        rootMatrixNode = matrixNode(0, zeroValIndex, "null","null", startState, "Start","null",cost_fn,0)
        ...
        while heap:
            ...
            if goalState == current_matrix:
                ...
                print("Steps: ")
                aStarMethodDumpFile.printSolutionSteps(temp_matrixNode)
                return 
            ...
            if current_matrix not in closed_set:
                ...
                for i in range(len(directions)):
                    ...
                    if self.directionIsSafe(new_zero_index):
                        ...
                        new_cost_fn = self.generateCostFunction(modified_matrix,goalState) + new_cost_gn
                        new_matrix_node = matrixNode(level+1,new_zero_index,temp_matrixNode,current_matrix,modified_matrix,new_action_val, changed_tile_val,new_cost_fn, new_cost_gn)
                        count += 1
